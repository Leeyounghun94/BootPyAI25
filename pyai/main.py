...
# pydantic : 데이터 검증, 직렬화, 타입 힌팅, 설정 관리
# Pillow : 이미지 열기, 저장, 변환, 다양한 이미지 처리용
# numpy : 수치계산, 배열 및 행렬 연산, 다양한 수학함수 제공
...
# 그 결과 이미즐 base64 인코딩된 문자열로 반환하는 서비스
...
# pydantic의 데이터 모델을 정의

import io       # 파일 입출력을 위한 모듈
import base64   # 데이터를 Base64로 인코딩 디코딩      https://ko.wikipedia.org/wiki/%EB%B2%A0%EC%9D%B4%EC%8A%A464 -> Base64
...
# YOLO8 모델 로드(yolov8n.pt 모델의 가중치 파일)

class DetectionResult(BaseModel):   # pydanstic을 사용하여 데이터 모델을 정의(응답 데이터를 구조화)
...
def detect_objects(image: Image):
    img = np.array(image)       # 이미지를 numpy 배열오 변환
